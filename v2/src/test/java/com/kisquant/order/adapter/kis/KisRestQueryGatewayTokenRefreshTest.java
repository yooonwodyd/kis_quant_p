...
import com.kisquant.marketdata.adapter.kis.KisRestQueryGateway;
...
class KisRestQueryGatewayTokenRefreshTest {

    private static final String BASE_URL = "https://kis.example";

    @Test
    void reissuesTokenAndRetriesQuoteOnceWhenKisReportsExpiredToken() {
        RestClient.Builder builder = RestClient.builder().baseUrl(BASE_URL);
        MockRestServiceServer server = MockRestServiceServer.bindTo(builder).build();
        RestClient restClient = builder.build();
        KisQuantProperties properties = properties();
        KisRestQueryGateway gateway = new KisRestQueryGateway(
                restClient,
        ...
        server.expect(requestTo(BASE_URL + "/uapi/domestic-stock/v1/quotations/inquire-price"
                        + "?FID_COND_MRKT_DIV_CODE=J&FID_INPUT_ISCD=001510"))
        ...
        server.expect(requestTo(BASE_URL + "/uapi/domestic-stock/v1/quotations/inquire-price"
                ...
                .andRespond(withSuccess("""
                        {"rt_cd":"0","output":{"stck_prpr":"72000"}}
                        """, MediaType.APPLICATION_JSON));

        Quote quote = gateway.quote(Symbol.of("001510"));

        assertThat(quote.currentPrice().amount()).isEqualTo(72_000L);
        server.verify();
    }
